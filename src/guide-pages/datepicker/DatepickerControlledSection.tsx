...
const DatepickerStateGuideProp = memo(function DatepickerStateGuideProp() {
  ...
  return (
    <GuideProp
      ...
      typeLabel="boolean"
      defaultValue="false"
      description="Datepicker는 기본적으로 직접 타이핑 입력이 막혀 있고, disabled는 전체 상호작용을 비활성화합니다. readOnly일 때는 현재 값만 표시하면서 캘린더 열기, 날짜 변경, clear 버튼을 막습니다."
      isWide
    >
      <Datepicker
...
export default function DatepickerControlledSection() {
  return (
    <GuideSection
      ...
      description="Datepicker는 selected와 onSelectedChange를 외부 상태로 관리하는 controlled usage를 기준으로 정리한 단일 날짜 선택 컴포넌트입니다. Textfield 기반 캘린더 입력 패턴으로 구성됩니다."
    >
