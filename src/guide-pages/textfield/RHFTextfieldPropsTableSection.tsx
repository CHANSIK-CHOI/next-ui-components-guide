...
import {
  RHF_TEXTFIELD_BASE_PROPS_CODE,
  TEXTFIELD_BASE_PROPS_CODE,
} from "@/components/Guide/textfieldTypeReferences";

const rhfTextfieldPropsRows: GuidePropsTableRow[] = [
  {
    name: "name",
  ...
  },
  {
    ...
    description:
      "required, validate, pattern 같은 검증 규칙을 useController 규격으로 전달합니다.",
  },
  {
    name: "defaultValue",
    typeLabel: "string",
  ...
  },
  {
    ...
    defaultValue: "false",
    description:
      "useController와 실제 Textfield에 동시에 반영되어 필드 업데이트와 상호작용을 막습니다.",
  },
  {
    name: "errorMessage",
    typeLabel: "string",
  ...
  {
    name: "placeholder / unit / isClearable / onClear ...",
    typeLabel: "Textfield inherited props",
    defaultValue: "inherited",
    description:
      "RHF가 직접 관리하지 않는 Textfield UI props와 native input props는 그대로 전달할 수 있습니다.",
  },
];

export default function RHFTextfieldPropsTableSection() {
  const rhfTextfieldTypeCode = `${RHF_COMPONENT_PROPS_CODE}
...
> &
  ...
  return (
    <GuideSection
      label="Props Table"
      title="RHFTextfield props 한눈에 보기"
      description="RHFTextfield에서 사용하는 RHF 제어 props와 Textfield 상속 props를 표로 정리했습니다."
    >
      <GuidePropsTable
        rows={rhfTextfieldPropsRows}
        note={
          <>
            <span>
              <code>RHFTextfieldProps</code>는{" "}
              <code>
                RHFComponentProps&lt;TFormValues, TFieldName, TextfieldProps,
                RHFValueInputManagedProps&gt;
              </code>
              ...
              나머지 Textfield UI props만 직접 전달하는 패턴입니다.
            </span>
            <div className="guidePropsTable__noteActions">
              <GuideTypeTooltip
                label="RHFTextfield 타입 보기"
                title="RHFTextfieldProps / RHFValueInputManagedProps"
                description="Textfield wrapper에서 RHF가 직접 관리하는 value 계열 prop 조합입니다."
                code={rhfTextfieldTypeCode}
              />
              <GuideTypeTooltip
                label="TextfieldBaseProps 보기"
                title="TextfieldBaseProps"
                description="RHFTextfield도 그대로 상속하는 Textfield의 커스텀 UI props입니다."
